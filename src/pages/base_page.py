...
class BasePage:
    ...
    agent_url = "https://interview.supporthive.com/staff/"
    support_center_url = "https://interview.supporthive.com/new/"
    ...
    def open_tab(self):
        ActionChains(config.DRIVER).key_down(Keys.CONTROL).send_keys('t').key_up(Keys.CONTROL).perform()

    def close_tab(self):
        config.DRIVER.find_element_by_tag_name("body").send_keys(Keys.CONTROL + "w")
    ...
    def hover(self, locator):
        element_to_hover_over = config.DRIVER.find_element_by_css_selector(locator)
        ActionChains(config.DRIVER).move_to_element(element_to_hover_over).perform()
    ...
    def wait(self,time=10):
        config.DRIVER.implicitly_wait(time)

    def file_upload(self, selector, path):
        config.DRIVER.find_element_by_css_selector(selector).send_keys(path)


    def click_hidden_element(self, selector):
        element = config.DRIVER.find_element_by_css_selector(selector)
        config.DRIVER.execute_script("$(arguments[0]).click();", element)


    def open_url_in_new_tab(self,url):
        config.DRIVER.execute_script(f"window.open('{url}' , 'new_window')")

    def switch_to_window(self,window_handle):
        config.DRIVER.switch_to.window(window_handle)

    def list_window_handles(self):
        window_handles = config.DRIVER.window_handles
